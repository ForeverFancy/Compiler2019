...
`bison`是采用自底向上方法进行语法分析的，再进行归约时，所有产生式右部的表达式的节点都已经建立好了，只需要新建产生式左部节点，并且添加子树。`yyparse()`函数中，如果需要读入下一个符号，它会调用`yylex()`，调用之后，如果这个`token`有`yylval`返回值，那么返回值将会被压入栈中，并且和`.y`文件中的语法产生式的`token`值绑定起来。如果语法产生式要执行`reduce`操作，那么将会执行产生式之后的动作。
