...
void CminusBuilder::visit(syntax_program &node) {
    assert(!node.declarations.empty());
...
void CminusBuilder::visit(syntax_param &node) {
    ...
    auto var = node.isarray ?
               builder.CreateAlloca(TyInt32Ptr) : builder.CreateAlloca(TyInt32);
...
void CminusBuilder::visit(syntax_var &node) {
    ...
    if (node.expression) {  // arr[exp] 数组元素访问
        ...
        llvm::Value *ptr;
        if (var_pointer->getType()->getPointerElementType()->isArrayTy()) {  // [n x i32]*
